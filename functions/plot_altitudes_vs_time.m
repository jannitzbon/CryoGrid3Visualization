function fig = plot_altitudes_vs_time( OUT , PARA )

    ts = OUT.timestamp();
    water_table = OUT.location.water_table_altitude();
    frost_table = OUT.location.active_layer_depth_altitude();
    ...
    axis( [ ts(1) ts(end) min(frost_table)-0.5 max(surface_level)+0.5 ] );
    %ax=gca;
